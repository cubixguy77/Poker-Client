...
public class Control implements ActionListener {
    ...
    private static ClientWindow gameWindow;
    ...
    public void handleUpdate(String[] info) {
        ...
        if (updateType.equals("chips")) {
            ...
            while (numToUpdate > 0) {
                if (players[Integer.parseInt(info[i*2 + 4])] != null) {
                    ...
                    gameWindow.updatePlayerChips(seat, chips);
                    gameWindow.updatePotSize(pot);
        ...
        /* Update contributed message encoded as #update#contributed#NumToUpdate#Seat#contr#seat#contr#...# */
            ...
            while (numToUpdate > 0) {
                if (players[Integer.parseInt(info[i*2 + 3])] != null) {
                    ...
                    if (contr > 0)
                        gameWindow.updateContributed(seat, contr);
                    else
                        gameWindow.removeContributedChips(seat);
         ...
         //   cardsVisible[seat] = true;
            ...
            gameWindow.updatePlayerCard(seat, getFilename(card1), 0);
            gameWindow.updatePlayerCard(seat, getFilename(card2), 1);
        }
         ...
         //   cardsVisible[seat] = false;
            ...
            gameWindow.removePlayerCards(seat);
        }
        else if(updateType.equals("removecards")) {
            myCards[0] = "";
            myCards[1] = "";
            
            gameWindow.removePlayerCards(mySeatID);
        }
            ...
            String message = players[seat].getUserName() + " is knocked out in " + place + "th place.";
            
            gameWindow.addMessage("Dealer", message);
            players[seat].setStatus(Player.Status.WATCHING_TABLE);
        }
        
        else if (updateType.equals("reward")) {
            int seat =   Integer.parseInt(info[2]);
            int place =  Integer.parseInt(info[3]);
            double reward = Double.parseDouble(info[4]);
            gameWindow.addMessage("Dealer", players[seat].getUserName() + " is rewarded $" + reward + ".");
        }
        
        else if (updateType.equals("win")) {
            int seat = Integer.parseInt(info[2]);
            String name = info[3];
            gameWindow.addMessage("Dealer", name + " has won the tournament.");
        }
        ...
        /* Update board message encoded as #update#board#card1#card2#...#card5#, blank cards included as ___ */
            ...
            for (int i=0; i<board.length; i++) {
                ...
                if ((info[i + 2].equals("___")))
                    gameWindow.clearCard(i);
                else                    
                    gameWindow.updateCard(getFilename(info[i + 2]), i);
            }
        }
            ...
            gameWindow.updatePlayerCard(mySeatID, getFilename(info[2]), 0);
            gameWindow.updatePlayerCard(mySeatID, getFilename(info[3]), 1);
        }
        
        else if (updateType.equals("handdealt")) {
            int seat = Integer.parseInt(info[2]);
            gameWindow.hidePlayerCards(seat);
            gameWindow.addMessage("Dealer", "Starting new hand, blinds at " + sbsize + "/" + bbsize + ".");
        }
            ...
            gameWindow.addMessage("Dealer", "Action is on player " + players[actionon].getUserName() + ".");
        }
            ...
            gameWindow.setAllButtons("Check", "Bet");
            gameWindow.setSlider(bbsize, players[mySeatID].getChipStack() - players[mySeatID].getContributedTotal());
        }
        else if (updateType.equals("fold")) {
            actionon = mySeatID; // the action is one me
            System.out.println("Fold, Call " + info[3] + ", or Raise " + info[5]);
            
            gameWindow.setAllButtons("Fold", "Call " + info[3], "Raise " + info[5]);
            gameWindow.setSlider(Integer.parseInt(info[5]), players[mySeatID].getChipStack() + players[mySeatID].getContributedThisStreet());
        }
            ...
            gameWindow.addMessage(speaker, message);
        }
            

        else if (updateType.equals("openGameWindow")) {
            gameWindow = new ClientWindow(this);
            gameWindow.setWindowTitle("Logged in as " + myName);
            gameWindow.updatePlayerName(mySeatID, myName);
        }
        else if (updateType.equals("closeGameWindow")) {
            gameWindow.setVisible(false);
            gameWindow.dispose();
        }
        
        else if (updateType.equals("windowTitle")) {
            gameWindow.setWindowTitle(info[2]);
        }
            ...
            for (int i=0; i<(info.length-2) / 3; i++) {
                ...
                gameWindow.updatePlayerName(seat, name);
                gameWindow.updatePlayerChips(seat, chips);
            }
        }
        ...
        /* Update pot message encoded as #update#pot#new pot amount# */
         ...
         /* Update button message encoded as #update#button#new button location# */
            ...
            gameWindow.addMessage("Dealer", "Blinds are up to " + sbsize + "/" + bbsize + ".");
        }
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() instanceof JButton) {
            ...
            gameWindow.clearTextField();                                               // so that everyone else at the table sees the message
        }                                                                                
        else { 
